...
### Generate normalized random matrix for random projection features.
def get_rp_matrix(dim_in, dim_out):

    return np.sqrt(dim_out) * np.random.randn(dim_in, dim_out)  #np.sqrt(dim_out) normalizes every column to 1
...
def get_matrix_generator(rand_mat_type, std, dim_kernel):
    ...
    elif rand_mat_type == "rp": return functools.partial(get_rp_matrix, dim_out = dim_kernel)
...
class Base:
    ...
    # Only Random Projection
    def convRP(self, X, index = None, chunk_size = 1024):
        W  = self.W if index is None else self.W[index]
        ts = X @ W
        return ts
